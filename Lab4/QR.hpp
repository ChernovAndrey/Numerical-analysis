...
private:
    ...
    vector<vector<T>>
    createTijMatrix(T c, T s, int ni, int nj, int strSize) { //содание Tij матрицы, strSize-кол-во строк
...
//прямой ход метода вращений; //flagB-есть ли вектор b
    void rotation(int ni, int nj, vector<vector<T>> &TMatrix, bool flagInitTMatrix,
                  bool flagB = false) {  //ni, nj -номер строк
